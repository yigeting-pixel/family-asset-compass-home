...
def normalize_current_holdings(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize uploaded or sample current holdings.
    ...
    if df is None or df.empty:
        return pd.DataFrame(columns=["code", "name", "amount", "cost_amount"])

    out = df.copy()
    out.columns = [str(c).strip() for c in out.columns]
    if "code" not in out.columns or "amount" not in out.columns:
        raise ValueError("当前持仓文件必须包含 code 和 amount 两列")

    out["code"] = out["code"].astype(str).str.zfill(6)
    out["amount"] = pd.to_numeric(out["amount"], errors="coerce").fillna(0.0)
    if "name" not in out.columns:
        out["name"] = out["code"]
    if "cost_amount" not in out.columns:
        out["cost_amount"] = out["amount"]
    out["cost_amount"] = pd.to_numeric(out["cost_amount"], errors="coerce").fillna(out["amount"])
    return out[out["amount"] > 0][["code", "name", "amount", "cost_amount"]]
...
def build_rebalance_plan(
    ...
    """Compare current holdings with target holdings and produce buy/sell/hold/watch actions."""
    ...
    for code in codes:
        ...
        if abs(diff) < min_trade_amount and abs(drift) < drift_threshold:
            ...
            trade_amount = diff
            reason = "当前配置低于目标比例"
            priority = "高" if abs(drift) >= 0.05 else "中"
        else:
            action = "卖出"
        ...
        if action == "卖出" and sell_loss_control and pnl_ratio < -0.15:
            action = "观察"
            trade_amount = 0.0
            reason = "当前浮亏较大，建议先人工复核是否分批调出或用新增资金稀释"
            priority = "中"
        ...
        if current_amount > 0 and target_amount == 0:
            if pnl_ratio < -0.15 and sell_loss_control:
                action = "观察"
                trade_amount = 0.0
                reason = "不在目标组合内，但浮亏较大，建议人工复核退出节奏"
                priority = "中"
            else:
                action = "卖出"
                trade_amount = current_amount
                reason = "不在目标组合内，建议逐步调出"
                priority = "高"
        ...
        rows.append({
            "基金代码": code,
            "基金名称": t.get("name") or c.get("name") or code,
...
def build_execution_batches(plan: pd.DataFrame, batch_count: int = 4) -> pd.DataFrame:
    ...
    for _, r in trades.iterrows():
        for b in range(1, batch_count + 1):
            if r["交易方向"] == "买入":
                condition = "按周/月定投执行；若市场快速上涨，不追高加速"
            else:
                condition = "按批调出；若持仓波动较大，优先卖出偏离度最高部分"
            rows.append({
                "批次": f"第{b}批",
                "基金代码": r["基金代码"],
                "基金名称": r["基金名称"],
                "交易方向": r["交易方向"],
                "本批金额": float(r["建议交易金额"]) / batch_count,
                "执行条件": condition,
            })
